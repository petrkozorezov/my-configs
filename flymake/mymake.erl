...
main(FileName) ->
	{ProjRoot, FilePath, proj_path} = 
 	...
 	file:set_cwd(filename:join(ProjRoot)),
  	files([filename:join(FilePath)]).


test() ->
	main("/home/petrovi4/Desktop/Workspaces/Erlang1/lego_game_server/src/utils.erl").
...
%%     io:format("Recompile: ~s\n",[File]),
	{ok, Pwd} = file:get_cwd(),
    compile:file(Pwd ++ "/" ++ File, [report_errors, report_warnings, error_summary |Opts]);
